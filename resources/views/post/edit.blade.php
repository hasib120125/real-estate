							...
							<div class="col-sm-12">
								<form class="form-horizontal" id="postForm" method="POST" action="{{ url()->current() }}" enctype="multipart/form-data">
									...
									<fieldset>
										...
										<div id="address" class="form-group required <?php echo (isset($errors) and $errors->has('address')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label" for="address">Address <sup>*</sup></label>
											<div class="col-md-8">
												<input id="address" name="address" placeholder=""
													   class="form-control input-md" type="text" value="{{ old('address') }}">
											</div>
										</div>
										<div id="handover_date" class="form-group required <?php echo (isset($errors) and $errors->has('handover_date')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label" for="handover_date">Handover Date <sup>*</sup></label>
											<div class="col-md-8">
												<input id="handover_date" name="handover_date" placeholder=""
													   class="form-control input-md" type="Date" value="{{ old('handover_date') }}">
											</div>
										</div>
										<div id="size" class="form-group required <?php echo (isset($errors) and $errors->has('size')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label" for="size">Size <sup>*</sup></label>
											<div class="col-md-8">
												<input id="size" name="size" placeholder=""
													   class="form-control input-md" type="text" value="{{ old('size') }}">
											</div>
										</div>
										<div id="video" class="form-group required <?php echo (isset($errors) and $errors->has('video')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label" for="video">Video <sup>*</sup></label>
											<div class="col-md-8">
												<input id="video" name="video" placeholder="Youtube Video Link"
													   class="form-control input-md" type="text" value="{{ old('video') }}">
											</div>
										</div>
										<!-- longitude -->
										<div id="longitude" class="form-group required <?php echo (isset($errors) and $errors->has('lon')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label" for="lon">Longitude <sup>*</sup></label>
											<div class="col-md-8">
												<input id="lon" name="lon" placeholder=""
													   class="form-control input-md" type="text" value="{{ old('lon',$post->lon) }}">
											</div>
										</div>

										<!-- latitude -->
										<div id="latitude" class="form-group required <?php echo (isset($errors) and $errors->has('lat')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label" for="lat">Latitude <sup>*</sup></label>
											<div class="col-md-8">
												<input id="lat" name="lat" placeholder=""
													   class="form-control input-md" type="text" value="{{ old('lat',$post->lat) }}">
												<div id='map_canvas' style="height: 350px;margin-top: 10px;"></div>
										...
										<!-- pictures -->
										<div id="picturesBloc" class="picturesBloc form-group <?php echo (isset($errors) and $errors->has('pictures')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label" for="pictures"> {{ t('Pictures') }} </label>
											<div class="col-md-8">
												<div class="mb10">
													<button class="btn btn-success"><i class="icon-camera-1"></i> {{ t('Upload pictures') }}</button>
												</div>
											</div>
										</div>
...
@section('after_scripts')
	...
	@if (file_exists(public_path() . '/public/assets/plugins/forms/validation/localization/messages_'.config('app.locale').'.min.js'))
		<script src="{{ url('public/assets/plugins/forms/validation/localization/messages_'.config('app.locale').'.min.js') }}" type="text/javascript"></script>
	@endif
    ...
    var map = new google.maps.Map(document.getElementById('map_canvas'), {
	    zoom: 15,
	    center: new google.maps.LatLng(parseFloat("{{ old('lat',$post->lat) }}"), parseFloat("{{ old('lon',$post->lon) }}")),
	    mapTypeId: google.maps.MapTypeId.ROADMAP
	});

	var myMarker = new google.maps.Marker({
	    position: new google.maps.LatLng(parseFloat("{{ old('lat',$post->lat) }}"), parseFloat("{{ old('lon',$post->lon) }}")),
	    draggable: true
	});

	google.maps.event.addListener(myMarker, 'dragend', function (evt) {
		document.getElementById('lat').value = evt.latLng.lat().toFixed(4);
		document.getElementById('lon').value = evt.latLng.lng().toFixed(4);
	    document.getElementById('current').innerHTML = '<p>Marker dropped: Current Lat: ' + evt.latLng.lat().toFixed(4) + ' Current Lng: ' + evt.latLng.lng().toFixed(4) + '</p>';
	});
	...
	<script src="{{ url('public/assets/js/app/d.select.category.js?v=' . vTime()) }}"></script>
	<script src="{{ url('public/assets/js/app/d.select.location.js?v=' . vTime()) }}"></script>
@endsection
