                                    ...
                                    <fieldset>
                                        @if (isset($postsPicturesNumber) and is_numeric($postsPicturesNumber) and $postsPicturesNumber > 0)
                                            ...
                                            <div id="picturesBloc" class="form-group <?php echo (isset($errors) and $errors->has('pictures')) ? 'has-error' : ''; ?>">
                                                <label class="col-md-3 control-label" for="pictures" style="text-align: left;"> {{ t('Pictures') }} </label>
                                                <div class="col-md-8"> </div>
                                                <div class="col-md-12" style="position: relative; float: right; padding-top: 10px; text-align: center;">
                                                    <div class="mb10">
...
@section('after_styles')
    <link href="{{ url('public/assets/plugins/bootstrap-fileinput/css/fileinput.min.css') }}" rel="stylesheet">
    <style>
...
@section('after_scripts')
    <script src="{{ url('public/assets/plugins/bootstrap-fileinput/js/plugins/sortable.min.js') }}" type="text/javascript"></script>
    <script src="{{ url('public/assets/plugins/bootstrap-fileinput/js/fileinput.min.js') }}" type="text/javascript"></script>
    @if (file_exists(public_path() . '/public/assets/plugins/bootstrap-fileinput/js/locales/'.config('app.locale').'.js'))
        <script src="{{ url('public/assets/plugins/bootstrap-fileinput/js/locales/'.config('app.locale').'.js') }}" type="text/javascript"></script>
    @endif
    <script>
        ...
        $('#pictureField').on('filebatchpreupload', function(event, data, id, index) {
            $('#uploadSuccess').html('<h4>{{ t('Upload Status') }}</h4><ul></ul>').hide();
        });
