	  ...
	  <div class="col-sm-4 no-padding photobox">					
        ...
        @endif
			...
			<div class="add-image">
				...
				<a href="{{ $postUrl }}">
					<img style="width: 100%;" class="thumbnail no-margin" src="{{ $postImg }}" alt="img">
				</a>
			</div>
		</div>

		<div class="col-sm-8 add-desc-box" style="padding: 0;">
			<div class="row land-details">
						<div class="col-md-7">
							<span class="info-row" style="padding:5px 0 10px 12px;">
					...
					@endif
						...
						<div class="col-md-5">
							<h5 class="add-title prpert-title"><a href="{{ $postUrl }}">{{ mb_ucfirst(str_limit($post->title, 70)) }} </a></h5>
						</div>
			</div>
			<div class="row" style="margin-bottom: 10px;">
			<div class="add-details col-sm-8">
				<div class="col-xs-5">
						<p>Location</p>
				</div>
				<div class="col-xs-7">
						<h5><span class="item-location"><i class="fa fa-map-marker"></i>&nbsp;
						<a style="font-size: 13px color:#666;" href="{!! qsurl(config('app.locale').'/'.trans('routes.v-search', ['countryCode' => $country->get('icode')]), array_merge(Request::except(['l', 'location']), ['l'=>$post->city_id])) !!}" class="info-link">{{ $city->name }}</a> {{ (isset($post->distance)) ? '- ' . round(lengthPrecision($post->distance), 2) . unitOfLength() : '' }}
					  </span></h5>
				</div>
				<div class="col-xs-5">
						<p>Slaes Price </p>
				...
				<div  class="col-xs-7">
					<h4 class="item-price" style="font-weight: 900;">
			...
			<div class="col-sm-3 text-right price-box">
					<div style="margin: 4px 0; clear: both;">
					<a style="color: #fff;" class="btn-primary details-fl btn btn-sm make-favorite" id="{{ $post->id }}" style="margin-bottom: 15px;">
								<span>Details More</span><i class=" fvv fa fa-asterisk"></i></a>
					</div>
					@if (Auth::check())
						
						<a style="color: #fff;" class="btn-primary details-fl btn btn-{{ (\App\Models\SavedPost::where('user_id', $user->id)->where('post_id', $post->id)->count() > 0) ? 'success' : 'default' }} btn-sm make-favorite"
						   id="{{ $post->id }}">
						   	<span>Add to Favourite</span>
							<i class=" fvv fa fa-heart"></i>
						</a>
					@else
						<a style="color: #fff;" class=" btn-primary details-fl btn make-favorite" id="{{ $post->id }}">
								<span>Add to Favourite</span><i class=" fvv fa fa-heart"></i></a>
					@endif
			...
			<div class="srch-footer">
				<div class="scale">
					<i class=" icn fa fa-bookmark" style="color: #222;"></i>
					<span class="icn-text" style="color: #222;">230 sq ft</span>
				</div>
				<div class="scale">
						<i class="fa fa-bed icn" style="color: #222;"></i>
						<span class="icn-text" style="color: #222;">2 bedrooms</span>
				</div>
				<div class="scale">
						<i class="fa fa-tint icn" style="color: #222;"></i>
						<span class="icn-text" style="color: #222;">2 baths</span>
				</div>
			</div>
		  </div>
		</div>
