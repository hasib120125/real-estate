		...
		<div class="user-panel-sidebar">
			...
			<div class="collapse-box">
				<h5 class="collapse-title">
					...
					<a href="#MyFriends" data-toggle="collapse" class="pull-right"><i class="fa fa-angle-down"></i></a>
				</h5>
				<div class="panel-collapse collapse in" id="MyFriends">
					<ul class="acc-list">
						<li>
							<a{!! ($pagePath=='friendlist') ? ' class="active"' : '' !!} href="{{ lurl('account/friendlist') }}">
							<i class="icon-user"></i> {{ t('My Friend Lists') }}&nbsp;
							<span class="badge">{{ isset($countMyFriends) ? $countMyFriends : 0 }}</span>
							</a>
						</li>
						<li>
							<a{!! ($pagePath=='favourite') ? ' class="active"' : '' !!} href="{{ lurl('account/favourite') }}">
							<i class="icon-heart"></i> {{ t('Favourite Properties') }}&nbsp;
							<span class="badge">{{ isset($countFavouritePosts) ? $countFavouritePosts : 0 }}</span>
							</a>
						</li>
						<li>
							<a{!! ($pagePath=='shared-properties') ? ' class="active"' : '' !!} href="{{ lurl('account/shared-properties') }}">
							<i class="icon-retweet"></i> {{ t('Shared Properties') }}&nbsp;
							<span class="badge">{{ isset($countSharedProperties) ? $countSharedProperties : 0 }}</span>
							</a>
						</li>
						<li>
