...
<div @include('admin::panel.inc.field_wrapper_attributes') >
	...
	<div class="btn-group" role="group" aria-label="..." style="margin-top: 3px;">
	  <button type="button" data-inputid="{{ $field['name'] }}-filemanager" class="btn btn-default popup_selector">
		<i class="fa fa-cloud-upload"></i> {{ trans('admin::messages.browse_uploads') }}</button>
		<button type="button" data-inputid="{{ $field['name'] }}-filemanager" class="btn btn-default clear_elfinder_picker">
		<i class="fa fa-eraser"></i> {{ trans('admin::messages.clear') }}</button>
	</div>
...
@if ($xPanel->checkIfFieldIsFirstOfItsType($field, $fields))
	...
	@push('crud_fields_styles')
		...
		<link href="{{ asset('public/vendor/admin/colorbox/example2/colorbox.css') }}" rel="stylesheet" type="text/css" />
		<style>
	...
	@push('crud_fields_scripts')
		...
		<script src="{{ asset('public/vendor/admin/colorbox/jquery.colorbox-min.js') }}"></script>
	@endpush
