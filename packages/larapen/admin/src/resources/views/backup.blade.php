...
@section('after_styles')
    ...
    <link href="{{ asset('public/vendor/admin/ladda/ladda-themeless.min.css') }}" rel="stylesheet" type="text/css" />
@endsection
...
@section('content')
    ...
    <div class="box">
        <div class="box-body">
            <button id="create-new-backup-button" href="{{ url(config('larapen.admin.route_prefix', 'admin').'/backup/create') }}" class="btn btn-primary ladda-button" data-style="zoom-in"><span class="ladda-label"><i class="fa fa-plus"></i> {{ trans('admin::messages.create_a_new_backup') }}</span></button>
            <br>
            ...
            <table class="table table-hover table-condensed">
                ...
                @foreach ($backups as $k => $b)
                    <tr>
                        <th scope="row">{{ $k+1 }}</th>
                        <td>{{ $b['disk'] }}</td>
                        <td>{{ \Date::createFromTimeStamp($b['last_modified'])->formatLocalized('%d %B %Y, %H:%M') }}</td>
                        <td class="text-right">{{ round((int)$b['file_size']/1048576, 2).' MB' }}</td>
                        <td class="text-right">
                            @if ($b['download'])
                                <a class="btn btn-xs btn-default" href="{{ url(config('larapen.admin.route_prefix', 'admin').'/backup/download/') }}?disk={{ $b['disk'] }}&path={{ urlencode($b['file_path']) }}&file_name={{ urlencode($b['file_name']) }}"><i class="fa fa-cloud-download"></i> {{ trans('admin::messages.download') }}</a>
                            @endif
                            <a class="btn btn-xs btn-danger" data-button-type="delete" href="{{ url(config('larapen.admin.route_prefix', 'admin').'/backup/delete/'.$b['file_name']) }}?disk={{ $b['disk'] }}"><i class="fa fa-trash-o"></i> {{ trans('admin::messages.delete') }}</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
...
@section('after_scripts')
    <!-- Ladda Buttons (loading buttons) -->
    <script src="{{ asset('public/vendor/admin/ladda/spin.js') }}"></script>
    <script src="{{ asset('public/vendor/admin/ladda/ladda.js') }}"></script>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $("[data-button-type=delete]").click(function(e) {
                ...
                if (confirm("{{ trans('admin::messages.backup.delete_confirm') }}") == true) {
                    $.ajax({
                        url: delete_url,
                        type: 'DELETE',
                        success: function(result) {
                            // Show an alert with the result
                            new PNotify({
                                title: "{{ trans('admin::messages.backup.delete_confirmation_title') }}",
                                text: "{{ trans('admin::messages.backup.delete_confirmation_message') }}",
                                type: "success"
                            });
