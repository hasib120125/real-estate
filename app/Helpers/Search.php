...
class Search
{
    ...
    public function __construct($preSearch = [])
    {
        ...
        // Priority setter
        if (Input::filled('distance') && is_numeric(Input::get('distance')) && Input::get('distance') > 0) {
    ...
    public function fetch()
    {
        ...
        if (Input::filled('type') && Input::get('type') != '') {
            $total = ($count->has(Input::get('type'))) ? $count->get(Input::get('type')) : 0;
        } else {
            $total = $count->get('all');
        }
                    ...
                    foreach($value as $fieldId => $postValue) {
                        if (is_array($postValue)) {
                            foreach($postValue as $optionId => $optionValue) {
                                ...
                                $this->arrSql->join[] = "INNER JOIN " . table('post_values') . " as ".$alias." ON a.id=".$alias.".post_id AND " . $where;
