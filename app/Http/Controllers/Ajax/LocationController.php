...
class LocationController extends FrontController
{
    ...
    public function getAdmin1WithCities($countryCode, Request $request)
    {
        ...
        $limit = 60;
        $homepage_area = $request->input('homepage_area');
        $cacheId = $countryCode . 'cities.where.subAdmin1.' . $adminCode . '.take.' . $limit;
        $cities = Cache::remember($cacheId, $this->cacheExpiration, function () use ($countryCode, $adminCode, $limit, $homepage_area) {
            if($homepage_area){
                $cities = City::countryOf($countryCode)
                ->where('subadmin2_code', $adminCode)->take($limit)
        ...
        if($request->input('homepage_city')){
            $option_cities = '<option value="">Select District</option>';
            foreach ($cities as $c_value) {
                $option_cities .= '<option value="'.$c_value->subadmin2_code.'">'.$c_value->asciiname.'</option>';
            }
            return $option_cities;
        }
        if($request->input('homepage_area')){
            $option_cities = '<option value="">Select Area</option>';
            foreach ($cities as $c_value) {
                $option_cities .= '<option value="'.$c_value->id.'">'.$c_value->asciiname.'</option>';
            }
            return $option_cities;
        }
