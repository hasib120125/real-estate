...
class DetailsController extends FrontController
{
    ...
    public function user_details(Request $request, $id){
        ...
        $user_query->with('allUserPosts')->where('id',$id);
        $key = trim(Input::get('key'));
        if($key != ''){
            $user_query->whereHas('allUserPosts',function($subQ) use ($key) {
                $subQ->where('title','LIKE','%'.$key.'%');
            });
        }
        $user_posts = $user_query->first();
        $users = User::with('allUserPosts')->where('id',$id)->first();
        $random_posts= Post::inRandomOrder()->take(3)->get();
        //dd($users->allUserPosts->userPostType);
        return view('post.user_details', compact('users','user_posts','key','random_posts','cats','data'));
    }
     ...
     */
    public function user_guideline(){
    ...
    public function user_connector(){
        return view('post.user-connector-list');
    }
