...
Route::group([
    ...
    Route::group(['middleware' => ['web', 'installChecker']], function($router) {
        ...
        Route::group(['namespace' => 'Post'], function($router) {
            ...
            Route::get('user/{id}', 'DetailsController@user_details');
            Route::get('{title}/{id}.html', 'DetailsController@index');
            Route::post('posts/{id}/contact', 'DetailsController@sendMessage');
            Route::get('{name}/connectors/{id}', 'DetailsController@user_connector');
              Route::get('guideline', 'DetailsController@user_guideline');
              Route::get('gallery', 'DetailsController@our_gallery');
              Route::get('blog', 'DetailsController@blog');
              Route::get('about-us', 'DetailsController@about_us');
        ...
        Route::group(['middleware' => ['auth', 'bannedUser', 'preventBackHistory'], 'namespace' => 'Account'], function($router) {
            ...
            $router->pattern('pagePath', '(my-posts|archived|favourite|pending-approval|saved-search|friendlist|shared-properties)+');
